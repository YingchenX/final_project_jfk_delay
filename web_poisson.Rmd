...
We would like to explore the relationship between cancellation and some predictors. The number of cancellations each day is therefore regarded as the result (dependent variable), and exposures of interest are regarded as the independent variables.
...
### Distribution of Daily Flight Cancellations
...
cancel_tidy %>% 
  ...
  labs(
    ...
    y = "Count",
    title = "Distribution of Daily Flight Cancellations",
...
We used the lag selection criteria and Akaike's information criterion (AIC) to select 6 days as the optimal time lag. Then, we constructed a new variable named `covid_lag6`, equivalent to the value of COVID-19 cases with a 6-day time lag.
...
poisson_by_airline %>% 
  ...
  knitr::kable(digits = 3, 
               ...
               caption = "Overview of estimated OR, 95% CI and P-value")
```
...
poisson_by_airline %>%
  filter(term != "(Intercept)") %>%
  mutate(text_label = str_c("Airline name: ", airline_name, "\nEstimated adjusted OR: ", OR)) %>% 
  plot_ly(
    x = ~airline_name, y = ~OR, type = "scatter", mode = "markers",
   text = ~text_label, alpha = 0.5, color = ~term, colors = "RdBu") %>% 
  layout(title = 'Estimated OR with 95% CI in Daily Flight Cancellation by Airline',
         xaxis = list(title = "Airline"),
         yaxis = list(title = "Estimated OR with CI"))
```
