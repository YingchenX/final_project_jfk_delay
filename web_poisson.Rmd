...
# Pre-analysis
## Check distribution
Since our outcome, `daily_cancel`, is calculated using a count variable (number of cancellation) repeated over time, we’ll model it using a Poisson regression model.
```{r}
...
cancel_covid <- cancel_tidy  %>%
  select(date, daily_cancel, covid_case) %>% 
  distinct()
