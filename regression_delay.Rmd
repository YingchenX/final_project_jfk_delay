...
* Disagreement: The scale of the outcome variable (delay in time) is from 0 to +infinity, whereas the scale of the linear function is from -infinity to +infinity. This disagreement will prevent us from applying a linear regression model to this data.
...
logdelay_hist = log_df %>% 
  ...
  geom_vline(aes(xintercept = mean(delay)), color = "blue",
             linetype = "dashed") +
  labs(title = "Distribution of Log delay in time, 2021/11/1 to 2022/1/31",
       x = "Log(delay time)", 
       y = "Count") +
  theme_classic()
...
\ 2. Collect a dataset with longer time intervals, currently, our data were collected between November 2021 to January 2022 which is a very short period to observe a predictable trend. A larger dataset with data collected on longer time intervals is needed to solve the problem.

\ 3. Modify the dependent variables, e.g., change to categorical delay in time, probably with multiple levels

\ 4. Modify the independent variables, e.g., humidity, which was observed a non-linear relationship with delay time, can be changed to ordinal data.
