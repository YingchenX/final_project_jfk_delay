...
#One-way ANOVA
res.anova <- Anova(lm(delay ~ airline, data = log_df))

anova_summary(res.anova, detailed = TRUE, effect.size = "ges") %>% mutate(p = format(p, scientific = TRUE, digits = 3)) %>% knitr::kable()
...
* Since the overall p-value (9.5e-81) is less than .05, this is an indication that each airline group does not have the same average delay time (in minutes).
